...
$exe = Join-Path $BuildDir "body-harvest-recomp.exe"
if (-not (Test-Path $exe)) {
    throw "no executable at $exe -- build first (see docs/BUILDING.md)"
}

$name = "body-harvest-recomp-$Version-windows-x64"
$stage = Join-Path $OutDir $name
if (Test-Path $stage) { Remove-Item -Recurse -Force $stage }
New-Item -ItemType Directory -Force $stage | Out-Null

$files = @(
    "$BuildDir\body-harvest-recomp.exe",
    "$BuildDir\SDL2.dll",
...
$pdb = Join-Path $BuildDir "body-harvest-recomp.pdb"
if (Test-Path $pdb) {
