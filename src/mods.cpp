// Mod content types this port adds. See include/bh/mods.h for what and why.
...
void publish() {
    ...
    std::fprintf(stderr, "[bh] texture packs: %zu enabled\n", paths.size());
    std::fflush(stderr);
}
...
void on_pack_enabled(recomp::mods::ModContext&, const recomp::mods::ModHandle& mod) {
    ...
    std::fprintf(stderr, "[bh] texture pack on: %s (%s)\n", id.c_str(), path.c_str());
    publish();
}
...
void on_pack_disabled(recomp::mods::ModContext&, const recomp::mods::ModHandle& mod) {
    ...
    std::fprintf(stderr, "[bh] texture pack off: %s\n", id.c_str());
    publish();
}
...
void register_content_types() {
    ...
    std::fprintf(stderr, "[bh] mod content type registered: rt64.json (texture pack)\n");
    std::fflush(stderr);
}
