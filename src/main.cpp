// Body Harvest: Recompiled -- entry point.
//
// Identifies a dump against the pinned target, and (with BH_WITH_RUNTIME and
// BH_WITH_RECOMPILED) registers the game with librecomp, supplies the platform
// callbacks ultramodern needs, and starts the runtime. Copied from Hybrid Heaven:
// Recompiled's main.cpp, itself Pilotwings 64's and Wave Race 64's.

#include "bh/crash_handler.h"
#include "bh/rom.h"
#if BH_WITH_RUNTIME && BH_WITH_RECOMPILED
...
// src/thread_sampler.cpp (BH_SAMPLE=1): where every thread is, every 2 s.
...
#ifndef BH_VERSION
#   define BH_VERSION "unknown"
#   define BH_VERSION_MAJOR 0
#   define BH_VERSION_MINOR 0
#   define BH_VERSION_PATCH 0
#endif
...
//
// On Windows this is %LOCALAPPDATA%\body-harvest-recomp. On macOS it is
// ~/Library/Application Support/body-harvest-recomp. On Linux and the rest it
// is $XDG_DATA_HOME/body-harvest-recomp, or ~/.local/share/body-harvest-recomp.
...
std::filesystem::path settings_directory() {
    ...
    return root / "body-harvest-recomp";
}
...
void print_usage(const char* argv0) {
    std::printf(
        ...
        "Body Harvest (USA) dump.\n",
        argv0, argv0, argv0);
}

void print_version() {
    std::printf("Body Harvest: Recompiled %s\n", BH_VERSION);
    std::printf("  recompiled game code : %s\n",
#if BH_WITH_RECOMPILED
        "linked");
#else
        "not built (configure with -DBH_WITH_RECOMPILED=ON)");
#endif
    std::printf("  runtime + RT64       : %s\n",
#if BH_WITH_RUNTIME
        "linked");
#else
        "not built (configure with -DBH_WITH_RUNTIME=ON)");
#endif
    std::printf("  frontend             : %s\n",
#if BH_WITH_FRONTEND
        "linked");
#else
        "not built (configure with -DBH_WITH_FRONTEND=ON)");
#endif
}
...
int identify(const char* path_arg) {
    ...
    if (!bh::read_header(path, header, error)) {
        std::fprintf(stderr, "error: %s\n", error.c_str());
        return 1;
    }

    std::printf("%-16s %s\n", "file",     path);
    std::printf("%-16s %s\n", "format",   bh::to_string(header.format).c_str());
...
void on_init(uint8_t* rdram, recomp_context* ctx) {
    ...
    std::printf("[bh] runtime initialised; entering recomp_entrypoint\n");
...
void on_thread_create(uint8_t* rdram, recomp_context* ctx) {
    (void)rdram;
    static int count = 0;
    std::printf("[bh] game thread %d created (entry 0x%08X)\n",
...
int run(int argc, char** argv, const char* rom_arg) {
    ...
    bh::load_input_script();

    if (rom_path != nullptr) {
        bh::RomHeader header;
        std::string error;
        if (!bh::read_header(rom_path, header, error)) {
            std::fprintf(stderr, "error: %s\n", error.c_str());
            return 1;
        }
        std::vector<std::string> problems;
    ...
    // Settings, controller profiles, mod state and the EEPROM save live in the
    // per-user folder; librecomp otherwise uses the working directory.
    {
        const std::filesystem::path settings = bh::settings_directory();
        std::error_code ec;
        std::filesystem::create_directories(settings, ec);
        recomp::register_config_path(settings);
        std::fprintf(stderr, "[bh] settings directory: %s\n", settings.string().c_str());
    }

    // The generated section tables reach librecomp before anything resolves an
    // address (src/overlays.cpp).
    bh::register_overlays();

    recomp::GameEntry game{};
    game.rom_hash = bh::kRomHash;
    game.internal_name = bh::kInternalName;
    game.display_name = bh::kDisplayName;
    game.game_id = bh::kGameId;
    game.mod_game_id = bh::kModGameId;
    // Body Harvest saves to a 4 Kbit EEPROM (docs/findings/phase-00.md).
    ...
    if (rom_path != nullptr) {
        const recomp::RomValidationError rom_error =
            recomp::select_rom(std::filesystem::path{rom_path}, std::u8string{bh::kGameId});
    ...
    config.project_version =
        recomp::Version{ BH_VERSION_MAJOR, BH_VERSION_MINOR, BH_VERSION_PATCH, "" };
    bh::inspector::init();
    ...
    bh::mods::register_content_types();
    config.rsp_callbacks = bh::rsp_callbacks();
#if BH_WITH_FRONTEND
    ...
    config.renderer_callbacks = bh::frontend::renderer_callbacks();
#else
    config.renderer_callbacks = bh::renderer_callbacks();
#endif
    config.audio_callbacks = bh::audio_callbacks();
    config.input_callbacks = bh::input_callbacks();
    config.gfx_callbacks = bh::gfx_callbacks();
    ...
    // verification runs use; BH_AUTOSTART=1 does the same with the stored dump
    // (src/frontend.cpp).
    if (rom_path != nullptr) {
        recomp::start_game(std::u8string{bh::kGameId}, std::string{});
    }
#else
    recomp::start_game(std::u8string{bh::kGameId}, std::string{});
#endif

    bh_start_thread_sampler();
    recomp::start(config);
    bh_stop_thread_sampler();
    std::fprintf(stderr, "[bh] recomp::start returned -- runtime shut down\n");
    std::fflush(stderr);

#if BH_WITH_FRONTEND
    bh::frontend::shutdown();
#endif

    bh::shutdown_platform();
    std::fprintf(stderr, "[bh] leaving main\n");
    std::fflush(stderr);
    return 0;
}

#endif  // BH_WITH_RUNTIME && BH_WITH_RECOMPILED

}  // namespace

int main(int argc, char** argv) {
    bh::install_crash_handler();
    ...
    // so a dump given as a relative path still resolves against it.
    {
        ...
        const std::filesystem::path exe_dir = bh::executable_directory(argv[0]);
        if (!exe_dir.empty()) {
            std::filesystem::current_path(exe_dir, ec);
            if (ec) {
                std::fprintf(stderr, "[bh] could not change to %s: %s\n",
    ...
    // and everything the port prints goes to bh.log in the settings directory
    ...
    // runtime treats as fatal on the first write (exit 0xC0000409).
    {
        ...
        if (GetConsoleWindow() != nullptr && GetConsoleProcessList(owners, 2) == 1) {
            const std::filesystem::path log = bh::settings_directory() / "bh.log";
            std::error_code ec;
            ...
            if (redirected) {
                std::fprintf(stderr, "[bh] output goes to this file when the game is started"
                                     " without a terminal\n");
            }
        }
    }
...
#if BH_WITH_FRONTEND && BH_WITH_RUNTIME && BH_WITH_RECOMPILED
    ...
    std::fprintf(stderr,
                 ...
                 "  -DBH_WITH_RUNTIME=ON -DBH_WITH_RECOMPILED=ON\n");
    return 1;
#endif
}
