...
RT64Context::RT64Context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle,
    ...
    app_config.appId = "body-harvest-recomp";
...
// Display lists submitted; update_screen reports the rate under BH_FRAME_STATS.
...
void RT64Context::send_dl(const OSTask* task) {
    ...
    if (dl_count == 0) {
        std::fprintf(stderr, "[bh] first display list: ucode 0x%08X data 0x%08X dl 0x%08X\n",
    ...
    //
    // Bisect switch for phase 04: setting BH_SKIP_DL isolates whether a fault
    // is inside RT64's display list processing or somewhere else entirely.
    static const bool skip_dl = std::getenv("BH_SKIP_DL") != nullptr;
    if (!skip_dl) {
        app_->processDisplayLists(app_->core.RDRAM, physical(task->t.data_ptr), 0, true);
    }

    if (dl_count == 1) {
        std::fprintf(stderr, "[bh] send_dl returned from the first display list\n");
        std::fflush(stderr);
    }
}
...
void RT64Context::update_screen() {
    if (valid_) {
        ...
        if (frames == 0 || frames == 60 || frames == 600) {
            std::fprintf(stderr, "[bh] update_screen #%llu\n",
                         static_cast<unsigned long long>(frames));
            std::fflush(stderr);
        }
        // BH_FRAME_STATS=1: display lists per 60 screen updates, which separates
        // "the game stopped drawing" from "the game draws an empty screen".
        static const bool stats = std::getenv("BH_FRAME_STATS") != nullptr;
        if (stats && frames % 60 == 0) {
            static uint64_t last = 0;
            const uint64_t now = dl_count.load();
            std::fprintf(stderr, "[bh] t=%llus display lists in the last 60 updates: %llu\n",
