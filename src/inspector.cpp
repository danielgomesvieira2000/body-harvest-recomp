// The HUD inspector. See include/bh/inspector.h for what it is for.
//
...
void draw_panel() {
    ...
    if (!ImGui::Begin("Body Harvest HUD")) {
        ImGui::End();
        return;
    }
    ...
    if (frame.number == 0) {
        // Nothing has called begin_frame: the feed is off (BH_INSPECTOR=0 would
...
void init() {
    ...
    // until RT64's UI is open. BH_INSPECTOR=0 turns the port's half off for
    // an A/B; RT64's own debug menu stays on F1 either way.
    const char* env = std::getenv("BH_INSPECTOR");
    g_enabled = (env == nullptr) || (env[0] != '0');
    if (!g_enabled) {
        std::fprintf(stderr, "[bh] HUD inspector off (BH_INSPECTOR=0)\n");
        std::fflush(stderr);
        return;
    }
    ...
    try {
        ...
        g_any_overrides.store(!g_overrides.empty(), std::memory_order_relaxed);
        std::fprintf(stderr, "[bh] HUD inspector: %zu class(es) loaded from %s\n",
                     g_overrides.size(), tag_path().string().c_str());
        std::fflush(stderr);
    }
    catch (const std::exception& e) {
        std::fprintf(stderr, "[bh] HUD inspector: could not read hud.json (%s); ignored\n", e.what());
        std::fflush(stderr);
    }
}
...
// Test hook: BH_TEST_HUD_OVERRIDE=<identity>=<class>@<seconds> sets an override
...
// spill.
void test_override() {
    static const char* spec = std::getenv("BH_TEST_HUD_OVERRIDE");
    ...
    for (int c = 0; c < 5; ++c) {
        if (name == names[c]) {
            ...
            g_any_overrides.store(true, std::memory_order_relaxed);
            std::fprintf(stderr, "[bh] test: HUD override %s -> %s\n", identity.c_str(), name.c_str());
            std::fflush(stderr);
        }
    }
    done = true;
...
void load_defaults([[maybe_unused]] std::unordered_map<std::string, int>& by_identity) {
    ...
    by_identity["tex:0x0025da40#38cb25f0"] = kLeft;
    by_identity["tex:0x0100a370#eb4a9ebf"] = kLeft;
    by_identity["tex:0x0100adf0#e46ed749"] = kLeft;
    // ---- end promoted ----
}
