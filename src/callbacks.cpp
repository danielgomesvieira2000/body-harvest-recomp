...
#include "bh/audiodiag.h"
#include "bh/crash_handler.h"
#include "bh/resample.h"
#if BH_WITH_FRONTEND
#   include "bh/frontend.h"
...
void refresh_players() {
    ...
    std::fprintf(stderr, "[bh] player 1 profiles: controller %d, keyboard %d\n",
                 recompinput::profiles::get_input_profile_for_player(
                     0, recompinput::InputDevice::Controller),
                 recompinput::profiles::get_input_profile_for_player(
                     0, recompinput::InputDevice::Keyboard));
    std::fprintf(stderr, "[bh] %zu controller%s connected; assigned to %zu player%s\n",
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
            case SDL_QUIT:
                ...
                std::fprintf(stderr, "[bh] SDL_QUIT received; asking the runtime to quit\n");
                std::fflush(stderr);
                ultramodern::quit();
                break;
...
bool get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
    ...
    if (!bh::frontend::capturing_input()) {
        ...
        // src/testdrive.cpp), so it is scaled to match.
        {
            ...
            bh::input_script_state(controller_num, &scripted_buttons, &scripted_x, &scripted_y);
    ...
    *y = stick_y;
    ...
    float scripted_x = 0.0f;
    float scripted_y = 0.0f;
    bh::input_script_state(&scripted_buttons, &scripted_x, &scripted_y);
    pressed |= scripted_buttons;
...
// It does that badly (see include/bh/resample.h), so the device is opened at
...
// BH_AUDIO_STATS at a 1024-frame period, the queue averaged 469 frames against
...
// shorter block was cleaner. The port converts now (see bh/resample.h), so
...
uint32_t queue_headroom_ms() {
    static const uint32_t value = [] {
        const char* env = std::getenv("BH_AUDIO_HEADROOM_MS");
...
int device_period_frames() {
    static const int value = [] {
        const char* env = std::getenv("BH_AUDIO_PERIOD");
...
bool resample_in_port() {
    static const bool on = std::getenv("BH_AUDIO_NO_RESAMPLE") == nullptr;
    return on;
}
...
bool open_audio_device() {
    ...
    if (SDL_InitSubSystem(SDL_INIT_AUDIO) != 0) {
        std::fprintf(stderr, "[bh] SDL_InitSubSystem(AUDIO) failed: %s\n", SDL_GetError());
        return false;
    }
    ...
    if (g_audio_device == 0) {
        std::fprintf(stderr, "[bh] SDL_OpenAudioDevice failed: %s\n", SDL_GetError());
        return false;
    }
    ...
    std::fprintf(stderr, "[bh] audio device open at %d Hz, %d channels,"
                         " %d-frame period, holding %u of the game's frames (%u ms) in hand\n",
                 have.freq, have.channels, have.samples,
                 g_queue_headroom_frames, queue_headroom_ms());
    std::fflush(stderr);

    bh::resample::configure(g_audio_frequency, g_device_frequency);
    ...
    bh::audiodiag::device_opened(g_device_frequency,
...
void queue_samples(int16_t* audio_data, size_t sample_count) {
    ...
    if (!announced_first) {
        ...
        std::fprintf(stderr, "[bh] first audio buffer queued: %zu frames at %u Hz\n",
    ...
    // device happens to run at the game's rate. See include/bh/resample.h.
    ...
    if (!announced) {
        for (size_t i = 0; i < sample_count; ++i) {
            if (unswapped[i] != 0) {
                announced = true;
                std::fprintf(stderr, "[bh] audio is audible (first non-silent buffer"
                                     " after %zu frames)\n", sample_count / 2);
                std::fflush(stderr);
                break;
            }
...
size_t get_frames_remaining() {
    ...
    // BH_AUDIO_STATS is set.
    bh::audiodiag::polled(static_cast<uint32_t>(device_frames));
    ...
    const size_t frames =
        static_cast<size_t>(bh::resample::to_source_frames(uint64_t(device_frames)));
...
RspExitReason rsp_unknown_ucode(uint8_t* rdram, uint32_t ucode_addr) {
    ...
    if (!reported) {
        reported = true;
        std::fprintf(stderr, "[bh] unrecognised RSP microcode at 0x%08X; "
...
// Body Harvest runs the same microcode, so the same defence applies. Copying
...
void bisect_audio_task(uint8_t* rdram, uint32_t ucode_addr) {
    ...
    for (uint32_t n = 1; n <= total && what == nullptr; ++n) {
        ...
        if (!top_reported && std::memcmp(&dmem[0xFC0], &copy, sizeof(OSTask)) != 0) {
            ...
            std::fprintf(stderr, "[bh-audio] (top of DMEM 0x%03X-0x%03X first written after command %u)\n",
        ...
        if (std::memcmp(&dmem[0x10], &pristine[0x10], 0x20) != 0) {
            ...
            std::fprintf(stderr, "[bh-audio] bytes changed in 0x000-0x040:");
            for (uint32_t i = 0; i < 0x40; ++i) {
    ...
    if (what == nullptr) {
        std::fprintf(stderr, "[bh-audio] every prefix ran clean on re-run: the failure depends on"
                             " state the first run changed, not on the command list alone\n");
        std::fflush(stderr);
        return;
    }
    std::fprintf(stderr, "[bh-audio] first prefix to damage %s is %u commands:"
    ...
    for (uint32_t i = from; i < bad + 2 && i < total; ++i) {
        ...
        std::fprintf(stderr, "[bh-audio]  %s%3u %-10s f=%02X a=%04X  b=%04X c=%04X   (%08X %08X)\n",
...
RspExitReason asp_main_watched(uint8_t* rdram, uint32_t ucode_addr) {
    ...
    bh::watch_for_hang("the audio microcode", 5);
...

    {
        ...
        if (std::memcmp(before.data(), rdram + original_base, g_audio_task_size) != 0) {
            ++changed_runs;
            if (changed_runs <= 3) {
                std::fprintf(stderr, "[bh] the game rewrote an audio command list while its task was running"
    ...
    if (reason != RspExitReason::Broke) {
        ...
        if (dropped <= 40 || dropped % 100 == 0) {
            std::fprintf(stderr, "[bh] audio frame dropped: the microcode did not reach"
...
RspUcodeFunc* get_rsp_microcode(const OSTask* task) {
    ...
    if (++others <= 5) {
        std::fprintf(stderr, "[bh] non-audio RSP task: type %u ucode 0x%08X\n",
...
void message_box(const char* msg) {
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Body Harvest: Recompiled", msg, g_window);
}
...
std::string get_game_thread_name(const OSThread* t) {
    ...
    // several platforms.
    char buf[16];
    std::snprintf(buf, sizeof(buf), "bh_%d", t != nullptr ? t->id : -1);
...
ultramodern::renderer::WindowHandle create_window(ultramodern::gfx_callbacks_t::gfx_data_t) {
    ...
    // BH_WINDOW_SIZE=WxH: a windowed test run at that size, whatever the settings
    ...
    int forced_w = 0;
    int forced_h = 0;
    if (const char* size = std::getenv("BH_WINDOW_SIZE")) {
    ...
    if (forced_w > 0) {
        width = forced_w;
        height = forced_h;
        std::fprintf(stderr, "[bh] window %dx%d (BH_WINDOW_SIZE)\n", width, height);
        std::fflush(stderr);
    }
        ...
        std::fprintf(stderr, "[bh] window %dx%d fullscreen (the display's own size)\n",
                     width, height);
        std::fflush(stderr);
    }
    else {
        ...
        if (SDL_GetDisplayUsableBounds(0, &usable) == 0) {
            ...
            width = 320 * scale;
            height = 240 * scale;
            std::fprintf(stderr, "[bh] window %dx%d (%dx upscale; display has %dx%d usable)\n",
                         width, height, scale, usable.w, usable.h);
            std::fflush(stderr);
        }
    }

    g_window = SDL_CreateWindow("Body Harvest: Recompiled",
    ...
    bh::frontend::publish_window(g_window);
#endif
...
void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t) {
    ...
    if (ticks == 0 || ticks == 1000 || ticks == 10000) {
        std::fprintf(stderr, "[bh] main loop tick %llu\n",
...
void set_audio_volume(double percent) {
    ...
    std::fprintf(stderr, "[bh] main volume: %d%%%s\n", clamped,
...
void shutdown_platform() {
    ...
    std::fprintf(stderr, "[bh] shutting down: window\n");
    std::fflush(stderr);
    if (g_window != nullptr) {
        SDL_DestroyWindow(g_window);
        g_window = nullptr;
    }
    std::fprintf(stderr, "[bh] shutting down: SDL\n");
    std::fflush(stderr);
    SDL_Quit();
    std::fprintf(stderr, "[bh] shutting down: done\n");
    std::fflush(stderr);
}
