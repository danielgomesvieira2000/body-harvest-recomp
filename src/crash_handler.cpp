...
LONG WINAPI on_exception(EXCEPTION_POINTERS* info) {
    ...
    std::fprintf(stderr, "\n[bh] ==== CRASH ====\n");
    std::fprintf(stderr, "[bh] %s (0x%08lX) at %p\n", exception_name(code),
    ...
    if (code == EXCEPTION_ACCESS_VIOLATION && record->NumberParameters >= 2) {
        const char* how = record->ExceptionInformation[0] == 0 ? "reading"
                        ...
                        : "executing";
        std::fprintf(stderr, "[bh] while %s address 0x%llX\n", how,
    ...
    if (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
        ...
        if (GetModuleFileNameA(module, path, sizeof(path)) != 0) {
            std::fprintf(stderr, "[bh] in module %s (base %p, offset 0x%llX)\n",
    ...
    // the executable, so "in body-harvest-recomp.exe" covers every recompiled game function
    ...
    if (symbols_ready) {
        ...
        if (SymFromAddr(process, reinterpret_cast<DWORD64>(pc), &displacement, symbol)) {
            std::fprintf(stderr, "[bh] in function %s + 0x%llX\n", symbol->Name,
    ...
    std::fprintf(stderr, "[bh] on thread %lu", GetCurrentThreadId());
    ...
    if (info->ContextRecord != nullptr) {
        ...
        std::fprintf(stderr, "[bh] rip %p, rsp %p\n",
        ...
        if (readable) {
            ...
            std::fprintf(stderr, "[bh] return addresses still on the stack:\n");
            int shown = 0;
...
void report_hang(HANDLE thread, const char* what, int seconds) {
    std::fprintf(stderr, "\n[bh] ==== HANG ====\n");
    std::fprintf(stderr, "[bh] %s has not returned after %d seconds\n", what, seconds);
...
extern "C" void bh_report_lookup_miss(unsigned int addr, void* return_address) {
    ...
    std::fprintf(stderr, "\n[bh] ==== LOOKUP MISS #%ld ====\n", index);
    std::fprintf(stderr, "[bh] no function registered at 0x%08X\n", addr);
    std::fprintf(stderr, "[bh] thread %lu\n", GetCurrentThreadId());
...
void install_crash_handler() {
    ...
    std::set_terminate([]() {
        std::fprintf(stderr, "\n[bh] ==== UNHANDLED EXCEPTION ====\n");
        if (std::current_exception()) {
            try {
                std::rethrow_exception(std::current_exception());
            } catch (const std::exception& e) {
                std::fprintf(stderr, "[bh] %s\n", e.what());
            } catch (...) {
                std::fprintf(stderr, "[bh] a non-std exception\n");
            }
        } else {
            std::fprintf(stderr, "[bh] terminate called with no active exception\n");
        }
        std::fprintf(stderr, "[bh] =============================\n");
        std::fflush(stderr);
        std::abort();
...
//
// The one that still says something useful says it: bh_report_lookup_miss
...
extern "C" void bh_report_lookup_miss(unsigned int addr, void* return_address) {
    std::fprintf(stderr, "[bh] function lookup failed at 0x%08X (caller %p)\n",
                 addr, return_address);
    std::fflush(stderr);
}
